...
The mic obviously also picks up the music itself, not just the crowd. There
are two ways this gets handled, in order of preference:

  1. **Adaptive echo cancellation** (`echo_cancel.EchoCanceller`) -- when the
     mixer can hand over a real reference waveform of what it just played
     (`AudioEngine.recent_output_mono`), `AudioInputWorker` runs the mic
     signal through an adaptive filter that models the actual speaker-to-mic
     acoustic path and subtracts its prediction sample-by-sample. This is
     the default whenever a reference is available.
  2. **Scalar bleed subtraction** (the original approach) -- if no reference
     waveform is available, fall back to subtracting a fixed fraction
     (`bleed_factor`) of the known playback *RMS* from the mic *RMS*. Cruder,
     but requires nothing more than a single number from the mixer, so it's
     kept as a no-dependencies fallback (e.g. `--no-echo-cancel`).
"""
...
class CrowdSensor:
    ...
    def update_from_residual(self, residual_rms: float, dt: float) -> str:
        """Like `update()`, but for a mic reading that's already had the
        known playback signal adaptively cancelled out of it (see
        `echo_cancel.EchoCanceller`) -- the residual is the crowd component
        directly, no RMS-based bleed subtraction needed on top of it."""
        return self.update(mic_rms=residual_rms, playback_rms=0.0, dt=dt)
...
class AudioInputWorker:
    ...
    def __init__(
        ...
        blocksize: int = 2048,
        stream_factory=None,
        reference_provider: Optional[Callable[[int], np.ndarray]] = None,
        ...
        # If both are given, mic blocks get run through adaptive echo
        # cancellation against a real reference waveform pulled from
        # `reference_provider` instead of the cruder RMS-bleed fallback.
        # `reference_provider(n)` should return the last `n` mono samples of
        # whatever the mixer just played, e.g. `AudioEngine.recent_output_mono`.
    ...
    def _callback(self, indata, frames, time_info, status):
        ...
        mic_mono = np.asarray(indata, dtype=np.float64).reshape(-1)

        if (
            self.echo_canceller is not None
            and self.reference_provider is not None
            and frames == self.echo_canceller.block_size
        ):
            ref_block = np.asarray(self.reference_provider(frames), dtype=np.float64).reshape(-1)
            residual = self.echo_canceller.process_block(mic_mono, ref_block)
            residual_rms = float(np.sqrt(np.mean(np.square(residual), dtype=np.float64)))
            self.sensor.update_from_residual(residual_rms, dt)
        else:
            mic_rms = float(np.sqrt(np.mean(np.square(mic_mono), dtype=np.float64)))
