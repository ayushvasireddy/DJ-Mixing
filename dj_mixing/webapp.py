...
def _start_worker(
    library_path: str, input_device, output_device, use_mic: bool, use_echo_cancel: bool = True
) -> None:
    try:
        with state.lock:
...
def api_start():
    ...
    use_echo_cancel = bool(data.get("use_echo_cancel", True))

    threading.Thread(
        target=_start_worker,
        args=(library_path, input_device, output_device, use_mic, use_echo_cancel),
        daemon=True,
    ).start()
