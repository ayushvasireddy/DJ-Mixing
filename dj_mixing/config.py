...
CROWD_PLAYBACK_BLEED_FACTOR = 0.6  # fallback bleed subtraction if AEC is off (see AudioInputWorker)
SUSTAINED_LOW_SECONDS = 20.0     # how long the crowd must stay LOW before we force a change

# --- Acoustic echo cancellation (crowd mic vs. known playback) -----------
# Adaptive filter that models the speaker-to-mic acoustic path and cancels
# it out of the mic signal in the waveform domain, replacing the cruder
# CROWD_PLAYBACK_BLEED_FACTOR scalar subtraction whenever a reference signal
# is available. See echo_cancel.py.
AEC_FILTER_TAPS = 4096  # ~93ms of acoustic path modeled at 44.1kHz
AEC_MU = 0.03
AEC_REGULARIZATION = 0.2
AEC_REFERENCE_HISTORY_SECONDS = 2.0
