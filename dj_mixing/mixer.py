...
class AudioEngine:
    ...
    def __init__(
        ...
        channels: int = 2,
        blocksize: int = 1024,
        reference_history_seconds: float = 2.0,
    ):
        self.samplerate = samplerate
        ...
        # Rolling mono history of what actually came out of `render()`, so
        # something listening to a mic pointed at the room can be handed a
        # real reference waveform (not just an RMS number) to adaptively
        # cancel out of its own signal. See echo_cancel.py.
        self._ref_buffer = np.zeros(max(1, int(reference_history_seconds * samplerate)), dtype=np.float32)
    ...
    def _push_reference(self, block: np.ndarray) -> None:
        """Slide the rolling reference buffer forward by a mono downmix of
        the block that was just rendered. Assumes the caller already holds
        `self._lock`."""
        mono = block.mean(axis=1).astype(np.float32)
        n = mono.shape[0]
        buf_len = self._ref_buffer.shape[0]
        if n >= buf_len:
            self._ref_buffer = mono[-buf_len:]
        else:
            self._ref_buffer = np.concatenate([self._ref_buffer[n:], mono])

    def recent_output_mono(self, n_frames: int) -> np.ndarray:
        """Return the most recent `n_frames` samples of mono output audio,
        for use as an echo canceller's reference signal. Zero-padded at the
        front if less than `n_frames` has been rendered yet."""
        with self._lock:
            buf = self._ref_buffer
            if n_frames <= buf.shape[0]:
                return buf[-n_frames:].copy()
            pad = np.zeros(n_frames - buf.shape[0], dtype=np.float32)
