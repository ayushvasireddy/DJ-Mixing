...
def test_start_spawns_worker_with_request_parameters(client, monkeypatch):
    ...
    def fake_worker(library_path, input_device, output_device, use_mic, use_echo_cancel):
        calls.append((library_path, input_device, output_device, use_mic, use_echo_cancel))
        with webapp.state.lock:
            webapp.state.status = "running"
    ...
    # use_echo_cancel wasn't in the request body -> defaults to True.
    assert calls == [("mylib", 1, 2, True, True)]


def test_start_passes_through_explicit_echo_cancel_toggle(client, monkeypatch):
    calls = []
    ...
    resp = client.post(
        "/api/start",
        json={"library": "mylib", "use_mic": True, "use_echo_cancel": False},
    )
    assert resp.status_code == 200
    ...
    assert calls == [("mylib", None, None, True, False)]
